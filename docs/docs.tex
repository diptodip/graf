...
\title{Architecture and Design of Graf}
...
\begin{abstract}
    This document describes the design, capabilities, and usage of \texttt{Graf}, software for counting objects in 3D images. The most typical use of \texttt{Graf} is to count single molecule mRNA in smFISH microscopy, where a \textit{C. elegans} embryo is imaged in layers (stacks). \texttt{Graf} uses a neural approach, whose architecture is summarized in this document.
\end{abstract}
...
\begin{figure}[!ht]
    ...
    \caption{Architecture of the neural network behind the Graf system for counting objects in 3D images.}
\end{figure}
